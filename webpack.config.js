...
SCHEMA_NAMES.forEach((schema_name) => {
  ...
  files.forEach((fileName) => {
    ...
    if (mode === "production") {
      entry[
        `${schema_name}/${fileNameWithoutExtension}`
      ] = `./src/${schema_name}/${fileName}`;
    } else {
      entry[
        `srv/resources/${schema_name}/${fileNameWithoutExtension}`
      ] = `./src/${schema_name}/${fileName}`;
    }
  });
});
...
module.exports = {
  ...
  module: {
    rules: [
        ...
        generator: {
          ...
          filename: function (data, assetInfo) {
            ...
            if (resourcePath[0] !== "src")
              throw new Error("Cannot get image outside ot src", resourcePath);
      ...
      {
        test: /\.(jpe?g|gif|png|svg|mp3|wav)$/i,
        use: [
          {
            loader: "file-loader",
            options: {
  ...
  plugins: [
    new CopyPlugin({
      ...
      patterns: SCHEMA_NAMES.map((schema_name) => ({
        ...
        to:
          mode === "production" ? schema_name : `srv/resources/${schema_name}`,
        filter: (f) =>
          !(
            f.endsWith(".tsx") ||
            f.endsWith(".jsx") ||
        ...
        transform: (content, path) => {
          if (path.match(/topic\./g) && path.endsWith(".json"))
            return json5ToJson(content);
          return content;
        },
